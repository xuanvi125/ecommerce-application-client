...
function Product() {
  ...
  useEffect(() => {
    async function fetchBook() {
      ...
      ProductService.getBooksByCategory(product?.category.id, 1, 5).then(
        (data) => {
          setSimilarProducts(() => {
      ...
      <div className="relative flex flex-col flex-wrap md:flex-nowrap justify-center bg-clip-border rounded-xl bg-white text-gray-700 shadow-md w-full max-w-[100%]  p-8">
        <div className="flex ">
          ...
          <div className="p-6 grow">
            ...
            <p className="block mb-2 font-sans text-base antialiased font-normal leading-relaxed text-gray-700">
              Publishing Year: <b>{product?.publishYear}</b>
            </p>
            <div className="flex gap-6 items-center">
              <div className="flex items-center gap-1">
                ...
                {Math.round(product?.ratingAverage * 10) / 10  || 4}
              </div>
            ...
            <div className="mb-8 grid grid-cols-5 gap-4">
              {similarProducts?.map((book) => (
                <Link key={book.id} to={`/user/product/${book.id}`}>
            ...
            <AddToCartButton bookId={product?.id} />
          </div>
        </div>
        <div>
          <Review bookId={product?.id} />
        </div>
      </div>
    </div>
  );
