...
export default function Dashboard() {
  ...
  useEffect(() => {
    const getRevenue = async () => {
      ...
      setSales(revenueFormat.format(statistics));
    };

    const getProducts = async () => {
      const statistics = await StatisticsService.getTotalProducts();
      setProducts(statistics);
    };
  ...
  useEffect(() => {
    const getTopProducts = async () => {
      ...
      const categories = statistics.map((item) => item.book.name);
    ...
    const getMonthlyRevenue = async () => {
      ...
      const revenue = statistics.map((item) => item.revenue);
